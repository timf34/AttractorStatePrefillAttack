...
class Turn:
    ...
    model: str | None = None  # which model produced a generated turn (cross-model pairings)
...
class Conversation:
    ...
    def as_dicts(self) -> list[dict]:
        return [{"speaker": t.speaker, "content": t.content, "origin": t.origin,
                 **({"model": t.model} if t.model else {})} for t in self.turns]
...
def run_selfplay(
    ...
    ``model_b`` (cross-model pairing): instance B is generated by a different
    model; ``model`` then only plays instance A. Each generated turn records
    ...
    while len(convo.turns) < num_turns:
        ...
        turn_model = model if speaker == "A" or model_b is None else model_b
        text = chat(client, turn_model, msgs, max_tokens=max_tokens, temperature=temperature)
        convo.turns.append(Turn(speaker=speaker, content=text, origin="generated", model=turn_model))
        if verbose:
